...
params:
  output_folders: !r c("/home/johannes/piktmp/remind/output/macro_2022-01-18_18.04.41/")
...
if(!all(dir.exists(params$output_folders))) stop("output_folders do not exist!")
fulldata_paths <- file.path(params$output_folders, "fulldata.gdx")
...
gdx_items <- list(list(name = "vm_welfareGlob"),
                  list(name = "v02_welfare"),
                  list(name = "pm_w"),
                  list(name = "vm_cons"),
                  list(name = "vm_Mport"),
                  list(name = "vm_Xport"),
                  list(name = "vm_cesIO"),
                  list(name = "vm_invMacro"),
                  list(name = "vm_costEnergySys"),
                  list(name = "vm_emiTe"))
...
my_data <- lapply(my_data, function(y) {
  ...
  if("tall" %in% colnames(y))     y <- filter(y, tall <= 2100)
  
  y %>% rename("scenarios" = run) %>% mutate(scenarios = substr(scenarios, 1, 20)) 
  })
```

...
update_info <- function(regionscode,revision) {
  ...
  content <- c('',
      ...
      '')
  lucode::replace_in_file(cfg$model,paste('*',content),subject)  
}
...
update_sets <- function(map) {
  ...
  content <- c(content,'      /',';') 
  lucode::replace_in_file('core/sets.gms',content,"SETS",comment="***")
}
...
lucode::replace_in_file(file    = "./modules/29_CES_parameters/load/datainput.gms",
...
if(!setequal(input_new, input_old) | cfg$force_download) {
  ...
  lucode::download_distribute(files        = input_new,
...
content <- c(content,'       /',paste0("       ",lucode::getModules("modules/")[,"name"]),'       /')
content <- c(content,'','module2realisation(modules,*) "mapping of modules and active realisations" /')
content <- c(content,paste0("       ",lucode::getModules("modules/")[,"name"]," . %",lucode::getModules("modules/")[,"name"],"%"))
content <- c(content,'      /',';')
lucode::replace_in_file('core/sets.gms',content,"MODULES",comment="***")
...
# Merge GAMS files
cat("\n################\nCreating full.gms ...")
lucode::singleGAMSfile(mainfile=cfg$model,output = path(cfg$results_folder, "full.gms"))
cat(" done.\n################\n\n\n")
...
cat("Preparation successfully completed.\n")

...
require(gdx, quietly = TRUE,warn.conflicts =FALSE)

manipulateFileWithCondition <- function(file, manipulations, ...) {
    if(!is.list(manipulations)) manipulations <- list(manipulations)
    f <- readLines(file)
    for (i in 1:length(f)) {
      for (m in manipulations) {
        if(grepl(m[3], f[i])) f[i] <- gsub(m[1], m[2], f[i], perl = TRUE)
      }
    }
  writeLines(f, file)
} 
...
prepare_and_run <- function() {
  ...
  cfg$gms$standAlone <- cfg$standAlone 
  cfg$gms$emulator_energySys <- cfg$emulator_energySys 
  ...
  if(cfg$standAlone != "off"){
    replace_in_file(file = 'core/datainput.gms',
  ...
  create_fixing_files <- function(cfg, input_ref_file = "input_ref.gdx") {
    ...
    if(cfg$model == "standalone/macro/main.gms"){
      cat("Creating macro stand alone fixing files...")
      create_macro_sa_fixings(cfg, ref_gdx_data)
    } else{
      cat("Creating standart fixing files...")
  ...
  create_macro_sa_fixings <- function(cfg, ref_gdx_data) {
  ...
  str_years_levs <- c("L \\('2005","L \\('2010","L \\('2015","L \\('2020","L \\('2025","L \\('2030",
                      "L \\('2035","L \\('2040","L \\('2045","L \\('2050","L \\('2055","L \\('2060",
                      "L \\('2070","L \\('2080","L \\('2090","L \\('2100","L \\('2110","L \\('2130",
                      "L \\('2150")
  str_years_margs <- gsub("L","M",str_years_levs)
  no_years  <- length(str_years_levs)
  
  # Write level values to file
  levs <- c()
  for (i in 1:no_years) {
    if(i < 3){
        levs <- c(levs, grep(str_years_levs[i], ref_gdx_data, value = TRUE))
    } else {
        levs <- c(levs, grep("(vm_costpollution|v02_inconvPen|vm_costTradePe).L", ref_gdx_data, value = TRUE))
    } 
  }
  
  writeLines(levs, "levs.gms")
  
  # Copy levs.gms and create fixings.gms.
  file.copy("levs.gms", "fixings.gms", overwrite = TRUE)
  ...
  for (i in 1:no_years) {
    if(i < 3){
        margs <- c(margs, grep(str_years_margs[i], ref_gdx_data, value = TRUE))
    } else {
        #margs <- c(margs, grep("(vm_(X|M)port|vm_costpollution).M", ref_gdx_data, value = TRUE))
    } 
  }
  writeLines(margs, "margs.gms")
  ...
  # equations
  margs_manipulateThis <- c(margs_manipulateThis, 
                            list(c("qm_budget.M","!!qm_budget.M")),
                            list(c("q_costEnergySys.M","!!q_costEnergySys.M")),
                            list(c("q_emiTe.M","!!q_emiTe.M")),
                            list(c("q24_costTradePe.M","!!q24_costTradePe.M")),
                            list(c("q01_balLab.M","!!q_balLab.M")),
                            list(c("q01_cesIO.M","!!q_cesIO.M")),
                            list(c("q01_prodCompl.M","!!q01_prodCompl.M")),
                            list(c("q01_kapMo.M","!!q01_kapMo.M")),
                            list(c("q01_kapMo0.M","!!q01_kapMo0.M")),
                            list(c("q01_invMacroAdj.M","!!q01_invMacroAdj.M")),
                            list(c("q01_limitShPpfen.M","!!q01_limitShPpfen.M")),
                            list(c("q01_limtRatioPpfen.M","!!q01_limtRatioPpfen.M")),
                            list(c("q01_invMacroAdj.M","!!q01_invMacroAdj.M")),
                            list(c("q02_welfareGlob.M","!!q02_welfareGlob.M")),
                            list(c("q02_welfare.M","!!q02_welfare.M")),
                            list(c("q80_budg_intertemp.M","!!q80_budg_intertemp.M")),
                            list(c("q80_costAdjNash.M","!!q80_costAdjNash.M")))

  # variables (trade in goods is freed below!)
  fixings_manipulateThis <- c(fixings_manipulateThis, 
                              list(c("vm_cons.FX", "!!vm_cons.FX")),
                              list(c("vm_cesIO.FX", "!!vm_cesIO.FX")),
                              list(c("vm_invMacro.FX", "!!vm_invMacro.FX")),
                              list(c("v01_invMacroAdj.FX", "!!v01_invMacroAdj.FX")),
                              list(c("vm_invRD.FX", "!!vm_invRD.FX")),
                              list(c("vm_invInno.FX", "!!vm_invInno.FX")),
                              list(c("vm_invImi.FX", "!!vm_invImi.FX")),
                              list(c("vm_damageFactor.FX", "!!vm_damageFactor.FX")),
                              list(c("vm_forcOs.FX", "!!vm_forcOs.FX")),
                              list(c("vm_costAdjNash.FX", "!!vm_costAdjNash.FX")),
                              list(c("vm_costEnergySys.FX", "!!vm_costEnergySys.FX")),
                              list(c("vm_emiTe.FX", "!!vm_emiTe.FX")))

  # Include fixings (levels) and marginals in full.gms at predefined position in core/loop.gms
  full_manipulateThis <- c(full_manipulateThis, 
                           list(c("cb20150605readinpositionforlevelfile",
                                  paste("first offlisting inclusion of levs.gms so that level value can be accessed",
                                        "$offlisting",
                                        "$include \"levs.gms\";",
                                        "$onlisting", sep = "\n"))),
                           list(c("cb20140305readinpositionforfinxingfiles",
                                  paste("offlisting inclusion of levs.gms, fixings.gms, and margs.gms",
                                        "$offlisting",
                                        "$include \"levs.gms\";",
                                        "$include \"fixings.gms\";",
  ...
  # Perform actual manipulation on levs.gms, fixings.gms, and margs.gms in one
  # parse through of the texts, respectively
  manipulateFile("levs.gms", levs_manipulateThis)
  manipulateFile("fixings.gms", fixings_manipulateThis)
  manipulateFile("margs.gms", margs_manipulateThis)
  
  # Free trade in goods!
  manipulateFileWithCondition("fixings.gms",
                              c(list(c("vm_Xport.FX", "!!vm_Xport.FX","vm_Xport.FX.*good")),
                                list(c("vm_Mport.FX", "!!vm_Mport.FX","vm_Mport.FX.*good"))))



  # Perform actual manipulation on full.gms, in one parse through of the text
  manipulateFile("full.gms", full_manipulateThis)
  }
...
# prepare_and_run <- function(){
#   cat("You submitted a job to slurm, well done!")
#   Sys.sleep(20)
# }

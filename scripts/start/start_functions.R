...
submit_remind_run <- function(cfg, wait_for = NULL) {
  ...
  # Change to run folder while in function
  ...
  if(cfg$slurmConfig=="direct") {
    ...
    if (!is.null(wait_for)){
      dependency <- create_dependency_string(wait_for)
      ...
      job_id <- system(paste0("sbatch --job-name=run_",cfg$title," --output=log.txt --mail-type=END --parsable --comment=REMIND --wrap=\"Rscript --no-site-file run.R \" ",cfg$slurmConfig), intern = TRUE)
    }
...
submit_remind_prepare <- function(cfg, wait_for = NULL) {
  ...
  if(cfg$slurmConfig=="direct") {
    ...
    if (!is.null(wait_for)){
      dependency <- create_dependency_string(wait_for)
      job_id <- system(paste0("sbatch --job-name=prepare_",cfg$title, dependency," --kill-on-invalid-dep=yes --output=prepare_log.txt --mail-type=FAIL --time=10 --parsable --comment=REMIND --wrap=\"Rscript prepare.R \"  --qos=priority --nodes=1 --tasks-per-node=1"), intern = TRUE)

    } else {
      job_id <- system(paste0("sbatch --job-name=prepare_",cfg$title," --output=prepare_log.txt  --mail-type=FAIL --time=10 --parsable --comment=REMIND --wrap=\"Rscript prepare.R \" --qos=priority --nodes=1 --tasks-per-node=1"), intern = TRUE)
    }
...
submit_clean_up <- function(copy_dir, wait_for){
  ...
  cmd <- paste0("sbatch --job-name=clean_up",dependency," --mail-type=FAIL --time=5 --parsable --comment=REMIND --wrap=\"rm -rf ",
                copy_dir,"\" --qos=standby --nodes=1 --tasks-per-node=1")
